...
func TestGenerateFromPetstoreJSON(t *testing.T) {
	// Create temporary directory for test output
	tmpDir := filepath.Join(os.TempDir(), "go-api-gen-test-json")
	defer os.RemoveAll(tmpDir)
	...
	config := Config{
		InputFile:   "../../examples/petstore.json",
		OutputDir:   tmpDir,
		PackageName: "client",
		ClientName:  "APIClient",
		Logger:      testLogger,
	...
	if err := gen.Generate(); err != nil {
		t.Fatalf("Failed to generate client from JSON: %v", err)
	}
	...
	if _, err := os.Stat(clientFile); os.IsNotExist(err) {
		t.Errorf("client.go was not generated from JSON")
	}

	if _, err := os.Stat(modelsFile); os.IsNotExist(err) {
		t.Errorf("models.go was not generated from JSON")
	}
}